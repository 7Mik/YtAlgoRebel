...
function runSafeZoneAlignment(likesEntries, myActivityEntries, playlistDislikes = []) {
  ...
  if (safeZoneLimitIndex !== -1) {
    ...
    logWarn("YtAlgoRebel: No overlap found between Liked Playlist and My Activity, and liked list is incomplete. Safe zone is empty. Skipping My Activity entries to prevent false positives.");
  }
...
test('Scenario 1: Complete likes list processes entire My Activity and marks unliked items as dislikes', () => {
  ...
  // Verification
  assert.strictEqual(result.safeZoneLimitIndex, myActivityEntries.length - 1, 'Safe zone limit index should be the last index of myActivityEntries');
...
test('Scenario 2: Incomplete likes list with overlap finds safe zone anchor and ignores older entries', () => {
  ...
  assert.strictEqual(result.safeZoneLimitIndex, 3, 'Safe zone limit index should anchor at index 3');
  assert.strictEqual(result.warnLogged, false, 'No warnings should be logged');
  ...
  assert.strictEqual(result.dislikesEntries.length, 2, 'Should only add the 2 dislikes within the safe zone');
...
test('Scenario 3: Incomplete likes list without overlap warns and skips My Activity processing', () => {
  ...
  const myActivityEntries = [
    { title: 'Disliked Video A' },
    { title: 'Disliked Video B' }
  ];
...
test('Scenario 4: Alphanumeric normalization correctly matches titles with minor differences', () => {
  ...
  normalizedResults.forEach((val, idx) => {
    assert.strictEqual(val, 'helloworld', `Variation "${variations[idx]}" should normalize to "helloworld"`);
  });
  ...
  assert.strictEqual(result.dislikesEntries.length, 0, 'Should match and filter out the liked video variation');
});

 ...
 */
export function scoreVideoKeywords(
  videoTitle,
  videoChannel,
  historyMap,
  likesMap,
  ...
  if (customPlaylistsData.length > 0 && customPlaylistsConfig.length > 0) {
    for (const plData of customPlaylistsData) {
      ...
      if (
        configPl &&
        configPl.weight !== undefined &&
        plData.keywordMap &&
        Object.keys(plData.keywordMap).length > 0
      ) {
...
function computeChannelScore(
  ...
  return (
    histChannelMatch * historyWeight +
